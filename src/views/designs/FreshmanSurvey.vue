...
import questionsData from '@/data/questionnares/freshman.json'
...
// --- 远程搜集配置 (LeanCloud) ---
// 1. 去 https://console.leancloud.app/ 注册账号 (国际版或国内版均可)
// 2. 创建应用 -> 设置 -> 应用凭证 -> 复制 AppID 和 AppKey 填入下方
const LC_APP_ID = '' 
const LC_APP_KEY = ''
// ------------------------------
...
function submitData() {
  ...
  const exportData = {
    ...
    analysis: mappedSpace.value
  }

  // 1. 自动上传到 LeanCloud (如果配置了)
  if (LC_APP_ID && LC_APP_KEY) {
    const url = `https://${LC_APP_ID.substring(0, 8)}.api.lncldglobal.com/1.1/classes/SurveyResult`
    ...
    .catch(err => {
      ...
      alert('网络连接失败，正在为您下载备份文件，请手动发送给负责人。')
      downloadJSON(exportData)
    })
  } else {
    // 2. 如果没配置云端，则仅下载文件
    alert('反馈已记录！即将下载 JSON 文件，请将其发送给负责人。')
    downloadJSON(exportData)
  }
}
...
function downloadJSON(data) {
  ...
  link.href = url
  link.download = `freshman-survey-${new Date().getTime()}.json`
...
<template>
  <div class="animaster-wrapper">
    <div class="container" id="app"> 
      <!-- Header --> 
      <header> 
        <div class="logo"><i class="fas fa-film"></i> AniMaster Freshman</div> 
      ...
      <div class="question-card"> 
        ...
        <div v-else-if="currentQuestion.type === 'result'" class="result-view">
           <div class="expert-badge" style="background: #74b9ff;">用户痛点分析</div> 
           <h2>{{ currentQuestion.title }}</h2> 
           <p>{{ currentQuestion.desc }}</p> 
           <p>根据您的回答，我们识别出您最关心的<b>“核心痛点维度”</b>：</p> 
            
           <div class="design-space-map"> 
             <div class="ds-card ds-d1" :class="{ highlight: mappedSpace.d1.length > 0 }"> 
               <span class="ds-title" style="color: #ff7675;">D1: 状态一致性痛点</span> 
               <div class="ds-content"> 
                 <b>您关注：</b><br> 
                 {{ mappedSpace.d1.length ? mappedSpace.d1.join('、') : '无明显痛点' }}
               </div> 
             </div> 
             <div class="ds-card ds-d2" :class="{ highlight: mappedSpace.d2.length > 0 }"> 
               <span class="ds-title" style="color: #a29bfe;">D2: 叙事逻辑痛点</span> 
               <div class="ds-content"> 
                 <b>您关注：</b><br> 
                 {{ mappedSpace.d2.length ? mappedSpace.d2.join('、') : '无明显痛点' }}
               </div> 
             </div> 
             <div class="ds-card ds-d3" :class="{ highlight: mappedSpace.d3.length > 0 }"> 
               <span class="ds-title" style="color: #fdcb6e;">D3: 美学控制痛点</span> 
               <div class="ds-content"> 
                 <b>您关注：</b><br> 
                 {{ mappedSpace.d3.length ? mappedSpace.d3.join('、') : '无明显痛点' }}
               </div> 
             </div> 
             <div class="ds-card ds-d4" :class="{ highlight: mappedSpace.d4.length > 0 }"> 
               <span class="ds-title" style="color: #55efc4;">D4: 交互体验痛点</span> 
               <div class="ds-content"> 
                 <b>您关注：</b><br> 
                 {{ mappedSpace.d4.length ? mappedSpace.d4.join('、') : '无明显痛点' }}
               </div> 
             </div> 
           </div> 
  
           <div style="margin-top:30px;"> 
             <button class="btn btn-next" @click="submitData">提交反馈 <i class="fas fa-check"></i></button> 
           </div> 
        </div>
...
.animaster-wrapper {
  --primary: #74b9ff; /* Changed to Blue for Freshman Theme */
  --secondary: #a29bfe; 
  --bg: #f0f8ff; /* AliceBlue bg */
  --text: #2d3436; 
  --card-bg: #ffffff; 
  --success: #00b894; 
  --accent: #0984e3;
...
.category-tag { 
  ...
  letter-spacing: 1px; 
  font-weight: 600; 
  margin-bottom: 0; /* Remove bottom margin to align with flex */
} 
...
.option-item.selected { 
  border-color: var(--primary); 
  background: rgba(116, 185, 255, 0.1); /* Lighter blue bg */
  font-weight: 600; 
} 
